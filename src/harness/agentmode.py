...
## 多模态（视觉问题）
当问题涉及"桌面有什么""屏幕里是什么""看看这个窗口/网页/图片"时：
1. 调用 `desktop_screenshot` 截图（网页/窗口类问题也可结合 `exec` 打开对应程序后再截图）；
2. 截图返回图片路径后，**立即**调用 `view_image` 加载并自己查看；
3. 基于你看到的内容作答；没有看到就如实说"我没看清"，必要时重新截图，不要编造画面。
...
CODING_PROMPT = (
    ...
    "- 需要查看 UI/运行效果时，用 `desktop_screenshot` + `view_image` 做视觉验证；\n"
...
MISSION_PROMPT = (
    ...
    "- 视觉验证优先用 `desktop_screenshot` + `view_image`；\n"
